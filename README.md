...
| **Week 8** — KNN & Tree Models | `StandardScaler` (slides 6–7), `KNeighborsClassifier` with k-tuning (1, 3, 5, 7, 11, 15, 21, 31 — slide 9 *"Best K for Loan Data"*), `DecisionTreeClassifier` with `GridSearchCV` over `max_depth ∈ [5, 30]` and `min_samples_split ∈ [20, 100]` (slide 16 — verbatim ranges), `RandomForestClassifier` with feature importance (slide 22) |
...
AI tools (Claude) were used during the coding, debugging, and feature engineering phases of this project. All code has been reviewed, understood, and tested by the student. Statistical methods and analysis decisions were made in alignment with DSA210 course content.
